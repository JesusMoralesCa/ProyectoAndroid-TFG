...
import com.example.tfg.GestionarAlumnosPermisosProfesorActivity.Companion.profesorEnviado

class AddAlumnosPermisosProfActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_alumnos_permisos_prof)

        AlumRecyclerView = findViewById(R.id.rvAlumADAddpp)
        AlumRecyclerView.layoutManager = LinearLayoutManager(this)
        AlumRecyclerView.setHasFixedSize(true)
        tvLoadingDataAlum = findViewById(R.id.tvLoadingDataAlumAddpp)
        AlumList = arrayListOf<Model>()
        AlumElegido = findViewById(R.id.AlumElegidoAddpp)
        getProfData()
    ...
    fun CallAñadirAlumnoAlumAdd(view: View){

        AñadirAlumnoAlumAdd()
    }


    private fun AñadirAlumnoAlumAdd() {
        // Obtén la instancia de la base de datos de Firebase
        val db = FirebaseFirestore.getInstance()

        // Define la ruta del documento a mover
        val rutaDocumentoDestino = "/Sensei/JesusMorales/Profesores/$profesorEnviado/Alumnos/$selectecAlum"
